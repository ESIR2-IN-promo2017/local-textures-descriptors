...
cv::Mat PatchMatchLab::apply(cv::Mat source, cv::Mat target, int iterations, int patchSize) {
	...
	for(int x=patchSize/2; x < out.cols-patchSize/2; x++) {
		for(int y=patchSize/2; y < out.rows-patchSize/2; y++) {
			...
			vector->x = dx;
			vector->y = dy;
			vector->dist = distanceLab(sourceChannels, targetChannels, dx, dy, x, y, patchSize);
		...
		if(forwardSearch) {
			...
			for(int x = patchSize/2; x < out.cols-patchSize/2; x++) {
				for(int y = patchSize/2; y < out.rows-patchSize/2; y++) {
					...
					double distLeft = distanceLab(sourceChannels, targetChannels, left->x, left->y, x, y, patchSize);
					...
					double distUp =  distanceLab(sourceChannels, targetChannels, up->x, up->y, x, y, patchSize);
			...
			for(int x = out.cols-patchSize/2; x >= patchSize/2; x--) {
				for(int y = out.rows-patchSize/2; y >= patchSize/2; y--) {	
					...
					double distRight = distanceLab(sourceChannels, targetChannels, right->x, right->y, x, y, patchSize);
					...
					double distDown = distanceLab(sourceChannels, targetChannels, down->x, down->y, x, y, patchSize);
		...
		for(int x = patchSize/2; x < target.cols-patchSize/2; x++) {
			for(int y = patchSize/2; y < target.rows-patchSize/2; y++) {
				...
				while (radius > 1) {
					...
					double dist = distanceLab(sourceChannels, targetChannels, random->x, random->y, x, y, patchSize);
					if (dist < outPtr->dist) {
						outPtr->x = random->x;
						outPtr->y = random->y;
						outPtr->dist = dist;
